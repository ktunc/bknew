...
$yayinda = "";
if($teknik_analiz['TeknikAnaliz']['yayinda'] == 1){
    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <h4>(<?php echo $teknik_analiz['TeknikAnaliz']['id']; ?>) TeknikAnaliz Düzenle</h4>
            </div>
            <div class="ibox-content">
                <form class="form-horizontal" id="form-haber">
                    ...
                    <div class="form-group"><label class="col-lg-2 control-label">Başlık:</label>
                        <div class="col-lg-10"><input type="text" name="baslik" placeholder="Başlık" class="form-control" value="<?php echo $teknik_analiz['TeknikAnaliz']['baslik']; ?>"></div>
                    </div>
                    <div class="form-group"><label class="col-lg-2 control-label">İçerik:</label>
                        <div class="col-lg-10">
                            <textarea name="icerik" class="summernote"><?php echo $teknik_analiz['TeknikAnaliz']['icerik']; ?></textarea>
                        </div>
                    </div>
                            ...
                            <div class="lightBoxGallery">
                                <section class="sortableblock">
                                    <div id="items" class="sortable">
                                        <?php foreach($teknik_analiz['TeknikAnalizResim'] as $row){
                                            echo '<div class="col-xs-12 col-sm-3 sortdata" data-ta-id="'.$row['teknik_analiz_id'].'" data-resim-id="'.$row['id'].'">';
                                            echo '<div class="thumbnail">';
                                            echo '<a href="'.$this->Html->url('/').$row['path'].'" data-gallery="" ><img src="'.$this->Html->url('/').$row['paththumb'].'" width="100%"></a>';
                                            echo '<div class="caption">';
                                            echo '<i onclick="FuncDeleteTeknikAnalizResim('.$row['id'].')" class="fa fa-trash fa-lg text-danger"></i>';
                                            echo '</div>';
                                            echo '</div>';
                                            echo '</div>';
...
//                                                echo '</li>';
                    ...
                    <input type="hidden" name="taId" value="<?php echo $teknik_analiz['TeknikAnaliz']['id']; ?>" />
                </form>
            </div>
        </div>
    </div>
...
//                                var resId = evt.item.dataset.resimId;
            ...
            setTimeout(function(){
                $.ajax({
                    ...
                    if(dat['hata']){
                        ...
                        $('form#form-haber input[name="haberId"]').val(dat['TeknikAnalizId']);
                        swal({
                            title: "Başarılı",
    ...
    function FuncDeleteTeknikAnalizResim(resId){
        swal({
            ...
            $.ajax({
                type:'POST',
                url:'<?php echo $this->Html->url('/');?>yoneticis/teknikanalizresimsil',
                data:{'resId':resId}
            }).done(function (data) {
                var dat = $.parseJSON(data);
                if(dat['hata']){
    ...
    function FuncTeknikAnalizResimSirala(){
        var resArr = [];
        $('div#items .sortdata').each(function () {
            resArr.push({'ta-id':$(this).data('ta-id'), 'resim-id':$(this).data('resim-id')});
        });
        $.ajax({
            type:'POST',
            url:'<?php echo $this->Html->url('/');?>yoneticis/teknikanalizresimsirala',
