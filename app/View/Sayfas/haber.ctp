...
?>
    <div class="container-fluid bg-white">
        <div class="col-sm-12 col-md-12">
            <div class="row">
                <p class="IlanHeader text-center"><?php echo $haber['Haber']['baslik']; ?></p>
            </div>
            <?php if(!empty($haber['HaberResim'])){ ?>
                <div class="row">
                    <section class="slider">
                        <div class="flexslider">
                            <ul class="slides" id="Gallery">
                                <?php
                                foreach($haber['HaberResim'] as $row){
                                    echo '<li>'
                                        .'<a href="'.$this->Html->url('/').$row['path'].'">'
                                        . '<img src="'.$this->Html->url('/').$row['path'].'" alt="'.$haber['Haber']['baslik'].' '.$row['id'].'"/>'
                                        .'</a>'
                                        . '</li>';
                                }
                                ?>
